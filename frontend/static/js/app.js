...
function syncThemeIcon() {
  if (themeButton) themeButton.setAttribute('aria-label', root.dataset.theme === 'dark' ? 'Ativar tema claro' : 'Ativar tema escuro');
}
syncThemeIcon();
...
function showToast(message, type = 'success') {
  ...
  close.type = 'button'; close.ariaLabel = 'Fechar notificação'; close.textContent = '×';
  toast.append(text, close, document.createElement('i'));
  region.append(toast); wireToast(toast);
}
...
document.addEventListener('click', (event) => {
  ...
  if (!input) return;
  const next = Math.min(Number(input.max) || 99, Math.max(Number(input.min) || 1, Number(input.value || 1) + Number(step.dataset.step)));
  input.value = next;
});
...
document.querySelectorAll('[data-phone-mask]').forEach((input) => {
  function formatPhone() {
    ...
    if (digits.length > 7) formatted = `${formatted.slice(0, 10)}-${digits.slice(7)}`;
...
document.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    document.querySelectorAll('[data-cart-count]').forEach((badge) => badge.textContent = data.count);
...
if (liveFilter) {
  ...
  async function updateCatalog() {
    const url = new URL(liveFilter.action || window.location.href, window.location.origin);
    ...
    try {
      ...
      if (nextGrid) document.querySelector('.product-grid').replaceWith(nextGrid);
